...
void cfg_init() {
    ...
    conf->pidfile = CONFIG_PIDFILE_DEFAULT;
    ...
    conf->ui_print_stats = CONFIG_UI_PRINT_STATS_DEFAULT;
    ...
    conf->rtlsdr_scan_enable = CONFIG_RTLSDR_SCAN_ENABLE_DEFAULT;
    conf->rtlsdr_scan_start_freq = CONFIG_SCAN_START_FREQ_DEFAULT;
    conf->rtlsdr_scan_end_freq = CONFIG_SCAN_END_FREQ_DEFAULT;
    conf->rtlsdr_scan_step_freq = CONFIG_SCAN_STEP_FREQ_DEFAULT;
...
void cfg_print() {
    ...
    ui_message("\n");
    ui_message("ui_print_stats:                %s\n", cfg_tochar_bool(conf->ui_print_stats));
    ui_message("\n");
    ...
    ui_message("\n");
    ui_message("rtlsdr_scan_enable:            %s\n", cfg_tochar_bool(conf->rtlsdr_scan_enable));
    ui_message("rtlsdr_scan_start_freq:        %zu\n", conf->rtlsdr_scan_start_freq);
    ui_message("rtlsdr_scan_end_freq:          %zu\n", conf->rtlsdr_scan_end_freq);
    ui_message("rtlsdr_scan_step_freq:         %zu\n", conf->rtlsdr_scan_step_freq);
    ui_message("\n");
...
int cfg_parse(int argc, char **argv) {
    ...
    *config_filename = '\0';
    ...
    while (1) {
        ...
        if (c == 'p') {
            ln = strlen(optarg) + 1;
            conf->pidfile = (char *) realloc((void *) conf->pidfile, sizeof(char) * ln);
            strcpy(conf->pidfile, optarg);
            continue;
        }
...
int cfg_parse_file(char *config_filename) {
    ...
    while (getline(&line, &line_size, fd) >= 0) {
        ...
        if (strcmp(param, "ui_print_stats") == 0) {
            conf->ui_print_stats = cfg_parse_flag(value);
            continue;
        }
        ...
        if (strcmp(param, "rtlsdr_scan_enable") == 0) {
            conf->rtlsdr_scan_enable = cfg_parse_flag(value);
            continue;
        }

        if (strcmp(param, "rtlsdr_scan_start_freq") == 0) {
            conf->rtlsdr_scan_start_freq = (uint32_t) strtol(value, &endptr, 10);
            continue;
        }

        if (strcmp(param, "rtlsdr_scan_end_freq") == 0) {
            conf->rtlsdr_scan_end_freq = (uint32_t) strtol(value, &endptr, 10);
            continue;
        }

        if (strcmp(param, "rtlsdr_scan_step_freq") == 0) {
            conf->rtlsdr_scan_step_freq = (uint32_t) strtol(value, &endptr, 10);
            continue;
        }
